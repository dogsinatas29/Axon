...
간단한 CLI 입력 프로그램이지만, 내부는 제약 기반 상태 머신과 검증 루프를 가진 복잡한 구조로 구성됩니다.
**[요구사항 명세서(spec.md)](./TEST/spec.md)**를 입력하면 Architect→Junior→Senior 오케스트레이션을 거쳐 검증된 코드가 실제 파일로 생성됩니다.
각 단계의 제안·리뷰·승인 과정을 사람이 작업 단위로 직접 확인하고 검증할 수 있습니다.
