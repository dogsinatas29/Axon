...
- **명세(Spec) → 작동하는 코드**: 요구사항을 즉시 기능적인 소스 코드로 변환합니다.
- **실행 전 검증**: 엄격한 IR 검증을 통해 환각 없는 논리적 결과물을 보장합니다.
- **실제 파일 생성**: 단순한 텍스트 제안이 아닌, 실제 물리적 파일을 프로젝트에 생성합니다.
- **전 공정 모니터링**: Architect → Junior → Senior 오케스트레이션의 모든 단계를 투명하게 확인합니다.
...
- [✨ 핵심 추가 기능](#핵심-추가-기능)
- [📋 추가 예정 기능](#추가-예정-기능)
  - [🍻 노가리 채널](#노가리-채널-lounge-system-nogarimd-추가-예정-기능)
  - [🎭 페르소나 기반 에이전트](#페르소나-기반-에이전트-추가-예정-기능)
  - [🤝 인사 게시판](#인사-게시판-추가-예정-기능)

## 🚀 언제 Axon을 쓰나요?
- **코드 생성의 신뢰성이 중요한 경우**: AI의 환각(Hallucination)을 배제하고 논리적으로 검증된 코드가 필요할 때.
- **여러 단계 검증이 필요한 시스템**: 물리적 파일 생성, 컴파일, 런타임 테스트까지의 자동화된 검증 루프가 필요할 때.
- **사람이 리뷰 가능한 AI 파이프라인이 필요한 경우**: Architect → Junior → Senior로 이어지는 투명한 공정을 직접 제어하고 싶을 때.
