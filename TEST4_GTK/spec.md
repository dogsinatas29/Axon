...
### 5. GtkDrawingArea Focusability 제약 조건
GTK4에서 `GtkDrawingArea`는 기본적으로 키보드 포커스를 가질 수 없는 위젯으로 설정되어 있습니다. 따라서 키 입력 및 IME가 올바르게 작동하려면 다음 설정을 반드시 수행해야 합니다.
1. `gtk_widget_set_focusable(drawing_area, TRUE)`를 명시적으로 호출하여 포커스 능력을 부여합니다.
2. `GtkGestureClick`을 이용하여 마우스 클릭 이벤트를 감지하고, 클릭 시 `gtk_widget_grab_focus(drawing_area)`를 호출하여 포커스를 강제로 가져오도록 연동합니다.
3. 애플리케이션 시작 시 초기 포커스를 에디터 캔버스에 기본 할당합니다.
...
<!-- AXON:SPEC:LUA
{
  ...
  "scripts": [
    {
      "file": "lua/init.lua",
      "role": "Lua runtime entrypoint, loads core modules"
    },
    {
      "file": "lua/core/editor.lua",
      "role": "Cursor position, viewport scroll state ownership"
    },
    {
      "file": "lua/core/renderer.lua",
      "role": "Render ordering, line indexing, viewport calculation"
    },
    {
      "file": "lua/core/motion.lua",
      "role": "h/j/k/l, w/b cursor movement mapping"
    }
  ],
  "c_bindings": [
    "draw_text", "set_color", "set_font_size",
    "get_canvas_width", "get_canvas_height", "get_line_height",
    "request_redraw", "get_buffer_line", "get_buffer_line_count"
  ]
}
-->
