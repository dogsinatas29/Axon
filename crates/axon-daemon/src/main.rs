    ...
    match cli.command {
        ...
        Commands::Run { resume } => {
            ...
            if fast_cfg.is_none() && resume {
                println!(
                    "⚠️  'axon_config.json' not found or invalid. Forcing Fast Resume with Default Config (Gemini-2.5-Flash)..."
                );
                fast_cfg = Some(AxonConfig {
                    arch_brand: "Gemini".to_string(),
                    arch_model: "gemini-2.5-flash".to_string(),
                    senior_brand: "Gemini".to_string(),
                    senior_model: "gemini-2.5-flash".to_string(),
                    senior_count: 1,
                    junior_brand: "Gemini".to_string(),
                    junior_model: "gemini-2.5-flash".to_string(),
                    junior_count: 1,
                });
            }

            let get_drv = |brand: &str,
                           model: &str|
            ...
            let (
                ...
                junior_model,
                junior_count_val,
            ) = if let Some(cfg) = &fast_cfg {
                println!("✅ Resuming factory operation from saved configuration...");
                (
                    get_drv(&cfg.arch_brand, &cfg.arch_model),
                    cfg.arch_model.clone(),
                    get_drv(&cfg.senior_brand, &cfg.senior_model),
                    cfg.senior_count,
                    get_drv(&cfg.junior_brand, &cfg.junior_model),
                    cfg.junior_count,
                )
            } else {
                let (architect_model, arch_brand, arch_name) = 'arch_recruit: loop {
                    ...
                    if let Ok(idx) = brand_val.parse::<usize>() {
                        if idx > 0 && idx <= available_models.len() {
                            ...
                            {
                                "Gemini" => Arc::new(axon_model::GeminiDriver::new(
                                    key.clone(),
                                    "list".into(),
                                ))
                                    as Arc<dyn axon_model::ModelDriver + Send + Sync>,
                                "Claude" => Arc::new(axon_model::ClaudeDriver::new(
                                    key.clone(),
                                    "list".into(),
                                ))
                                    as Arc<dyn axon_model::ModelDriver + Send + Sync>,
                                "ChatGPT" => Arc::new(axon_model::OpenAIDriver::new(
                                    key.clone(),
                                    "list".into(),
                                ))
                                    as Arc<dyn axon_model::ModelDriver + Send + Sync>,
                            ...
                            if let Ok(models) = driver.list_available_models().await {
                                ...
                                loop {
                                    ...
                                    println!(
                                        "❌ Invalid version. Please choose 1-{}.\n",
                                        models.len()
                                    );
                                }
                            }
                ...
                let new_cfg = AxonConfig {
                    arch_brand,
                    arch_model: arch_name.clone(),
                    senior_brand,
                    senior_model: s_name,
                    senior_count: senior_count_val,
                    junior_brand,
                    junior_model: j_name,
                    junior_count: junior_count_val,
                };
                let _ = std::fs::write(
                    "axon_config.json",
