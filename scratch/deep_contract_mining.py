...
# Update get_semantic_risks to include 'expected' and 'detected' for rejection risks
start_idx = -1
...
if start_idx != -1 and end_idx != -1:
    new_func = [
        ...
        "    let mut global_ir: Option<axon_ir::ArchitectureIR> = None;\n",
        "\n",
        ...
        "                        if \u0026comp.file_path == target_file {\n",
        "                            expected = format!(\"Component: {}\\nTier: {:?}\\nContracts: {}\", \n",
        "                                comp.name, comp.tier, \n",
        "                                comp.contracts.iter().map(|c| c.name.clone()).collect::\u003CVec\u003C_\u003E\u003E().join(\", \"));\n",
        "                            break;\n",
        "                        }\n",
        "                    }\n",
        "                }\n",
...
with open(path, "w") as f:
    f.writelines(lines)
print("Successfully patched server.rs for deep contract mining in rejections")
