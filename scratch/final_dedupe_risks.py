...
if start_idx != -1 and end_idx != -1:
    # Final unified logic: track projects with pending approvals to hide redundant risks
    clean_func = [
        ...
        "\n",
        "    // v0.0.30: [UNIFIED_FILTER] Track projects that already have a pending approval (Green Card)\n",
        ...
        "                        projects_with_approvals.insert(project_id.to_string());\n",
        "                        temp_risks.push(approval.clone());\n",
        "                    }\n",
        "                }\n",
        "            }\n",
        "        }\n",
        "        if let Some(ir) = crate::intelligence::decision::load_project_ir(\u0026path.to_string_lossy()) {\n",
        "            let project_id = ir.project_id.clone();\n",
        ...
        "                // Add project_id to the risk object for later filtering\n",
        ...
        "    }\n",
        "\n",
        "    // v0.0.30: [FINAL_DEDUPE] Filter out risks if a project already has a pending approval (Green Card)\n",
        "    for r in temp_risks {\n",
        "        let project_id = r[\"project_id\"].as_str().unwrap_or(\"\");\n",
        "        let is_approval = r.get(\"approved\").is_some();\n",
        "        \n",
        "        if is_approval {\n",
        "            risks.push(r);\n",
        "        } else if !projects_with_approvals.contains(project_id) {\n",
        "            risks.push(r);\n",
        "        }\n",
        "    }\n",
        "\n",
    ...
    with open(path, "w") as f:
        f.writelines(lines)
    print("Successfully implemented final deduplication in get_semantic_risks")
else:
