...
# Update get_semantic_risks to include "Sacred Contract" and "Failed Stage"
start_idx = -1
...
if start_idx != -1 and end_idx != -1:
    new_func = [
        ...
        "            if raw_log.contains(\"cmake\") || raw_log.contains(\"error:\") { \n",
        "                actor = \"CMake Verification\"; \n",
        "                failed_stage = \"Build/Linking\";\n",
        "            } else if raw_log.contains(\"Senior\") { \n",
        "                actor = \"Senior Review\"; \n",
        "                failed_stage = \"Semantic Audit\";\n",
        "            }\n",
        "\n",
        "            // Extract Expected Contract from IR\n",
        "            let mut expected_contract = \"No contract found in IR\".to_string();\n",
        "            let mut detected_symbol = \"Unknown\".to_string();\n",
        ...
        "                    if let Some(comp) = ir.components.iter().find(|c| c.1.name.contains(target)) {\n",
        "                        let funcs: Vec\u003CString\u003E = comp.1.functions.iter().map(|f| f.1.name.clone()).collect();\n",
        "                        expected_contract = format!(\"Required Symbols: [{}]\", funcs.join(\", \"));\n",
        "                        // Try to find what was actually in the code to show diff-like info\n",
        "                        if let Some(code) = \u0026last_code {\n",
        "                           if !funcs.is_empty() \u0026\u0026 !code.contains(\u0026funcs[0]) {\n",
        "                               detected_symbol = \"Symbol mismatch or missing implementation\".to_string();\n",
        "                           }\n",
        "                        }\n",
        "                    }\n",
        "                }\n",
...
with open(path, "w") as f:
    f.writelines(lines)
print("Successfully patched server.rs with Governance-First logic")
