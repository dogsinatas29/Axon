...
if start_idx != -1 and end_idx != -1:
    # High-integrity logic: filter out ghost risks and ensure risk_id presence
    clean_func = [
        ...
        "        \n",
        "        // v0.0.30: Improved project_id derivation to avoid ghost '.' projects\n",
        ...
        "                            // Ensure risk_id exists for decision handling\n",
        ...
        "                    obj.insert(\"project_id\".to_string(), serde_json::Value::String(project_id.clone()));\n",
        "                    // Ensure risk_id is present to avoid 422 errors on decision submit\n",
        "                    if obj.get(\"risk_id\").is_none() {\n",
        ...
        "        \n",
        "        // v0.0.30: Strict filter to remove ghost '.' projects and redundant risks\n",
        "        if r_project_id == \".\" || r_project_id == \"..\" { continue; }\n",
        "\n",
        "        if is_approval {\n",
        "            risks.push(r);\n",
    ...
    with open(path, "w") as f:
        f.writelines(lines)
    print("Successfully hardened get_semantic_risks against ghost projects and schema errors")
else:
